      ...
      <table class="table table-bordered" id="order-dataTable" width="100%" cellspacing="0">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ strlen($review->review) > 50 ? substr($review->review, 0, 50) . '...' : $review->review }}</td>
            <td class="table-flex">
            ...
            <td>
              <a href="{{route('review.edit',$review->id)}}" class="btn btn-primary btn-sm float-left mr-1"
                style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit"
                data-placement="bottom"><i class="fas fa-edit"></i></a>
              <form method="POST" action="{{route('review.destroy',[$review->id])}}">
                @csrf
                @method('delete')
                <button class="btn btn-danger btn-sm dltBtn" data-id={{$review->id}} style="height:30px;
