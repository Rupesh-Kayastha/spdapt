    ...
    <tr>
      <td class="content">
              ...
              @foreach (json_decode($orderDetails, true) as $orderDetail)
                <tr class="table-font">
                  ...
                  <td>{{--<img src="{{ url('/public/product/') }}/{{ $cart_photo[0] }}" class="img-fluid zoom"
                      style="max-width:80px" alt="{{ $cart_photo[0] }}">--}}
                      <img src="https://staging.morshgolf.com/public/product/{{ $cart_photo[0] }}" class="img-fluid zoom"
                      style="max-width:80px" alt="{{ $cart_photo[0] }}">
                    </td>
        ...
        @if ($billing->ship_to_different == 1)
          ...
          <p>Postcode/ZIP: {{ $billing->post_code_shipping }}</p>
        @endif
